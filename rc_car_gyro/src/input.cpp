...
void input_do()
{
  steering_in_value = pulseIn(STEERING_IN_PIN, HIGH, PULSEIN_TIMEOUT);
  throttle_in_value = pulseIn(THROTTLE_IN_PIN, HIGH, PULSEIN_TIMEOUT);
  if (steering_in_value == 0 || throttle_in_value == 0)
  {
    failsafe = true;
  }
  else
  ...
  aux_in_value = pulseIn(AUX_IN_PIN, HIGH, PULSEIN_TIMEOUT) > MID_SIG_TO_CONTROL;
